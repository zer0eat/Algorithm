...
sheep, wolf = 0, 0                                                          # 양과 늑대의 수를 셀 변수를 생성하고
        ...
        if field[i][j] != '#' and visited[i][j] == 0:                       # 해당원소가 울타리가 아니고 방문전이라면
            ...
            while lst:                                                      # lst가 빌때까지 반복해서
                ...
                if tmp_sheep > tmp_wolf:                                    # 구역내 양의 수가 늑대의 수보다 많다면
                    sheep += tmp_sheep                                      # 양의 수에 구역 내 양의 수를 더하고
                else:                                                       # 늑대의 수가 양의 수보다 같거나 크다면
                    wolf += tmp_wolf                                        # 늑대의 수에 구역 내 늑대의 수를 더한다
print(sheep, wolf)                                                          # 모든 탐색을 마친 후 남아있는 양과 늑대의 수를 출력한다