...
for t in range(T):                                          # 테스트 케이스 수만큼 반복
    ...
    miro = [list(map(int, input())) for n in range(N)]      # 길이가 N인행렬을 miro에 input
    ...
    for i in range(N):                                      # 미로의 행의 길이 만큼 반복
        for j in range(N):                                  # 미로의 열의 길이 만큼 반복
            if miro[i][j] == 3:                             # 도착점을 찾았다면
                a = i                                       # 행을 a에 저장
                b = j                                       # 열을 b에 저장

    if a != 0 and miro[a - 1][b] == 4:                      # 맨 위 행렬이 아니면서, 윗칸의 요소가 4이면
        print(f'#{t+1} 1')                                  # 시작점과 도착점이 연결된 것이므로 탈출 가능인 1을 출력
    elif b != N - 1 and miro[a][b + 1] == 4:                # 맨 오른쪽 행렬이 아니면서, 오른쪽칸의 요소가 4이면
        print(f'#{t+1} 1')                                  # 시작점과 도착점이 연결된 것이므로 탈출 가능인 1을 출력
    elif b != 0 and miro[a][b - 1] == 4:                    # 맨 왼쪽 행렬이 아니면서, 왼쪽칸의 요소가 4이면
        print(f'#{t + 1} 1')                                # 시작점과 도착점이 연결된 것이므로 탈출 가능인 1을 출력
    elif a != N - 1 and miro[a + 1][b] == 4:                # 맨 아래 행렬이 아니면서, 아래칸의 요소가 4이면
        print(f'#{t+1} 1')                                  # 시작점과 도착점이 연결된 것이므로 탈출 가능인 1을 출력
