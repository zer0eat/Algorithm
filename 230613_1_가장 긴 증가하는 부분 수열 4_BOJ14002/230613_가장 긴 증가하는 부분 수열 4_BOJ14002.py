...
ans = [0]                               # 가장 긴 증가하는 부분수열을 저장할 리스트를 생성하고 가장 작은값보다 작은 값을 넣어 기준점을 생성한다
...
answer = deque()                        # 가장 긴 증가하는 부분 수열을 저장할 deque 생성
...
for i in range(A-1, -1, -1):            # 수열을 역순으로 반복해서
    if visited[i] == order:             # 해당 순서의 원소가 나오면
        answer.appendleft(lst[i])       # answer에 원소를 appendleft한 후
        order -= 1                      # 순서를 1 감소한다
